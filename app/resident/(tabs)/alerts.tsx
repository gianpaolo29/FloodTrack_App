...
// ─── AlertCard ───────────────────────────────────────────────────────────────
function AlertCard({ alert, isDark, onPress, animValue }: {
  alert: AlertItem; isDark: boolean; onPress: () => void; animValue: Animated.Value;
}) {
  ...
  const cardBg  = !alert.read ? (isDark ? accentColor + '0A' : accentColor + '08') : baseBg;
  const titleColor = alert.read ? (isDark ? colors.slate[400] : colors.slate[500]) : (isDark ? colors.white : colors.slate[900]);
  const translateX = animValue.interpolate({ inputRange: [0, 1], outputRange: [-32, 0] });
  ...
  return (
    <Animated.View style={{ opacity: animValue, transform: [{ translateX }] }}>
      <Pressable
        ...
        style={({ pressed }) => [
          styles.card,
          { backgroundColor: cardBg, borderWidth: alert.read ? 1 : 1.5, borderColor: alert.read ? (isDark ? 'rgba(255,255,255,0.06)' : 'rgba(0,0,0,0.05)') : accentColor + '35' },
      ...
      >
        <View style={styles.cardInner}>
          <View style={styles.cardTopRow}>
            <LinearGradient colors={[accentColor + '28', accentColor + '10']} style={styles.iconBadge}>
              <Ionicons name={iconName} size={22} color={accentColor} />
            </LinearGradient>
            <View style={styles.cardTitleWrap}>
              <Text style={[styles.cardTitle, { color: titleColor }]} numberOfLines={2}>{alert.title}</Text>
            </View>
            <View style={styles.cardRight}>
              {!alert.read && <PulseDot color={accentColor} />}
              <View style={[styles.timePill, isDark && { backgroundColor: colors.dark.surface }]}>
                <Text style={[styles.timeText, isDark && { color: colors.slate[400] }]}>{alert.time}</Text>
              </View>
            </View>
          </View>
          <Text style={[styles.cardBody, isDark && { color: colors.slate[400] }]} numberOfLines={2}>{alert.body}</Text>
          <View style={styles.cardFooter}>
            <View style={styles.footerLeft}>
              <Ionicons name="location-outline" size={12} color={colors.slate[400]} />
              <Text style={[styles.footerAreaText, isDark && { color: colors.slate[500] }]} numberOfLines={1}>{alert.area || 'Unknown area'}</Text>
            </View>
            <LinearGradient colors={[accentColor + '28', accentColor + '14']} style={styles.kindBadge}>
              <Text style={[styles.kindText, { color: accentColor }]}>{isCritical ? 'Critical' : isStatusUpdate ? 'Update' : 'Advisory'}</Text>
            </LinearGradient>
          </View>
        </View>
      </Pressable>
...
// ─── SectionLabel ─────────────────────────────────────────────────────────────
function SectionLabel({ label, count, color }: { icon: keyof typeof Ionicons.glyphMap; label: string; count: number; color: string; isDark: boolean }) {
  return (
...
export default function AlertsScreen() {
  ...
  const runEntranceAnims = useCallback((count: number) => {
    ...
    const cardSequences = cardAnims.slice(0, Math.min(count, 30)).map((anim, i) =>
      Animated.sequence([Animated.delay(i * 55), Animated.spring(anim, { toValue: 1, friction: 7, tension: 70, useNativeDriver: true })]),
    );
    Animated.parallel([
      Animated.parallel([
      ...
      Animated.sequence([Animated.delay(160), Animated.timing(listOpacity, { toValue: 1, duration: 320, useNativeDriver: true })]),
  ...
  useEffect(() => {
    ...
    const sub = AppState.addEventListener('change', (state) => { if (state === 'active') load(true); });
  ...
  return (
    <View style={[styles.root, { backgroundColor: screenBg }]}>
      ...
      <Animated.View style={[styles.headerWrap, { opacity: heroOpacity, transform: [{ translateY: heroTransY }] }]}>
        ...
        <Animated.View style={[StyleSheet.absoluteFillObject, { transform: [{ translateX: listTranslate }] }]}>
          {loading && (
            <View style={styles.centered}>
              <LinearGradient colors={[colors.gradients.cta[0] + '20', colors.gradients.cta[1] + '20']} style={styles.loadingIconWrap}>
          ...
          {!loading && error && (
            <View style={styles.centered}>
              <LinearGradient colors={[colors.gradients.cta[0] + '20', colors.gradients.cta[1] + '20']} style={styles.emptyIconWrap}>
              ...
              <Pressable onPress={() => load()} style={({ pressed }) => [styles.retryBtn, pressed && { opacity: 0.85, transform: [{ scale: 0.97 }] }]}>
          ...
          {!loading && !error && (
            <Animated.View style={[styles.listWrapper, { opacity: listOpacity }]}>
              <ScrollView
                contentContainerStyle={[styles.scroll, { paddingBottom: insets.bottom + 108 }, alerts.length === 0 && styles.scrollEmpty]}
                refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => { setRefreshing(true); load(true); }} tintColor={colors.gradients.cta[0]} colors={[colors.gradients.cta[0]]} />}
                showsVerticalScrollIndicator={false}
              >
                {criticals.length > 0 && (
                  <View style={styles.section}>
                    <SectionLabel icon="alert-circle" label="Critical Alerts" count={criticals.length} color={colors.severity.critical} isDark={isDark} />
                    {criticals.map(a => {
                      const slot = cardSlot++;
                      return <AlertCard key={a.id} alert={a} isDark={isDark} onPress={() => handleAlertPress(a)} animValue={cardAnims[Math.min(slot, 29)]} />;
                    })}
                  </View>
                )}
                {nonCriticals.length > 0 && (
                  <View style={styles.section}>
                    <SectionLabel icon="notifications-outline" label="Advisories & Updates" count={nonCriticals.length} color={isDark ? colors.gradients.cta[0] : colors.slate[600]} isDark={isDark} />
                    {nonCriticals.map(a => {
                      const slot = cardSlot++;
                      return <AlertCard key={a.id} alert={a} isDark={isDark} onPress={() => handleAlertPress(a)} animValue={cardAnims[Math.min(slot, 29)]} />;
                    })}
                ...
                {alerts.length === 0 && (
                  <View style={styles.emptyState}>
                    ...
                    <Text style={[styles.emptySub, isDark && { color: colors.slate[400] }]}>No active alerts right now.{'\n'}Critical incidents will appear here immediately.</Text>
                    <Pressable onPress={() => load()} style={({ pressed }) => [styles.retryBtn, pressed && { opacity: 0.85, transform: [{ scale: 0.97 }] }]}>
                      <LinearGradient colors={colors.gradients.cta} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} style={styles.retryBtnGrad}>
        ...
        <Animated.View style={[StyleSheet.absoluteFillObject, { transform: [{ translateX: detailTranslate }] }]}>
          {selectedAlert && (
            <AlertDetail
              ...
              onBack={closeDetail}
              onViewReport={selectedAlert.reportId ? () => { closeDetail(); router.push(`/resident/report/${selectedAlert.reportId}`); } : undefined}
            />
          )}
        </Animated.View>
      </View>
...
const styles = StyleSheet.create({
  ...
  waveWrap: { height: 52, position: 'relative', marginTop: -1 },
  waveShape: { position: 'absolute', bottom: 0, left: -12, right: -12, height: 56, borderTopLeftRadius: 28, borderTopRightRadius: 28 },
  ...
  section: { gap: 10 },
  sectionRow: { flexDirection: 'row', alignItems: 'center', gap: 9, marginBottom: 4, paddingLeft: 2 },
  sectionPill: { width: 3, height: 14, borderRadius: 1.5 },
  sectionLabel: { flex: 1, fontSize: 12, fontWeight: '700', textTransform: 'uppercase', letterSpacing: 0.8 },
  sectionCount: { paddingHorizontal: 9, paddingVertical: 3, borderRadius: 10 },
  sectionCountText: { fontSize: 11, fontWeight: '800' },

  card: { borderRadius: 18, overflow: 'hidden', shadowColor: '#000', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.08, shadowRadius: 16, elevation: 5 },
  cardInner: { flex: 1, padding: 14, gap: 9 },
  cardTopRow: { flexDirection: 'row', alignItems: 'flex-start', gap: 11 },
  iconBadge: { width: 46, height: 46, borderRadius: 16, alignItems: 'center', justifyContent: 'center', flexShrink: 0 },
  cardTitleWrap: { flex: 1 },
  cardTitle: { fontSize: 15, fontWeight: '700', lineHeight: 20 },
  cardRight: { alignItems: 'flex-end', gap: 7, flexShrink: 0 },
  timePill: { backgroundColor: colors.slate[100], paddingHorizontal: 8, paddingVertical: 3, borderRadius: 8 },
  timeText: { fontSize: 10, fontWeight: '600', color: colors.slate[500] },
  cardBody: { fontSize: 13, color: colors.slate[500], lineHeight: 20 },
  ...
  kindBadge: { paddingHorizontal: 9, paddingVertical: 4, borderRadius: 9 },
  kindText: { fontSize: 10, fontWeight: '800', textTransform: 'uppercase', letterSpacing: 0.5 },

  emptyState:    { alignItems: 'center', gap: 18 },
  emptyIconWrap: { width: 92, height: 92, borderRadius: 30, alignItems: 'center', justifyContent: 'center', marginBottom: 4 },
