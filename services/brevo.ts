...
export async function sendEmailVerificationOtp(toEmail: string, otp: string): Promise<void> {
  if (!API_KEY) throw new Error('Brevo API key is missing. Add EXPO_PUBLIC_BREVO_API_KEY to your .env and restart Expo.');
...
<head>
  ...
  <meta name="viewport" content="width=device-width,initial-scale=1.0"/>
  <meta name="x-apple-disable-message-reformatting"/>
  <title>Verify Your FloodTrack Email</title>
  <!--[if mso]>
  <noscript><xml><o:OfficeDocumentSettings><o:PixelsPerInch>96</o:PixelsPerInch></o:OfficeDocumentSettings></xml></noscript>
  <![endif]-->
</head>
<body style="margin:0;padding:0;background-color:#0A0F1E;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,Helvetica,Arial,sans-serif;-webkit-font-smoothing:antialiased;">
              ...
              <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0">
                <tr>
                  <td style="padding:44px 44px 36px;">
                    ...
                    <p style="margin:0 0 28px;font-size:15px;color:#718096;line-height:1.7;">
                      Thanks for signing up for FloodTrack! Please verify your email address
                      <strong style="color:#4A6CF7;font-weight:700;">${toEmail}</strong>
                      by entering the code below in the app.
                    </p>
                    ...
                    <!-- Security notice -->
                    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0"
                      style="margin-top:28px;border-radius:14px;overflow:hidden;
                             background:#FFFBEB;border:1px solid #FDE68A;">
                      <tr>
                        <td style="padding:16px 20px;">
                          <table role="presentation" cellpadding="0" cellspacing="0" border="0">
                            <tr>
                              ...
                              <td>
                                <p style="margin:0;font-size:13px;color:#92400E;line-height:1.6;">
                                  <strong>Didn't sign up?</strong> If you didn't create a FloodTrack account,
  ...
  const res = await fetch(BREVO_API_URL, {
    ...
    body: JSON.stringify({
      sender: { name: SENDER_NAME, email: SENDER_EMAIL },
      to: [{ email: toEmail }],
      subject: '✉️ Verify Your FloodTrack Email',
      htmlContent: html,
    }),
  });
